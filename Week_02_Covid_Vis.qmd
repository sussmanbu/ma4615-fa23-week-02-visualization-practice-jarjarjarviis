...
```{r}
covid_data_count |>
  ggplot(aes(x=state, y = Deaths, color = race)) +geom_line()+theme(axis.text.x = element_text(angle = 65, hjust = 1))

```

2. Are there any conclusions you can draw?
Arizona has the most death counts
3. What were you not able to do due to not having the R knowledge?
I wouldnt know the specific data group in visual
4. What other data would be useful to better understand this data?
I think the vaccine rate is also pretty important
